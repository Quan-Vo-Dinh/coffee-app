...
import {
  AUTH_TYPE_KEY,
  AuthMetadataPayload,
} from 'src/libs/decorators/auth.decorator';
    ...
    const guards = authTypeValue.authTypes.map((type) => {
      ...
      if (!guard) {
        throw new UnauthorizedException(
          `No guard found for auth type: ${type}`,
        );
      }
      return { type, guard };
    });

    const results: { type: string; success: boolean; error?: any }[] =
      await Promise.all(
